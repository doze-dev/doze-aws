...
func New(opts Options) (*Server, error) {
	...
	db, err := lazybolt.Open(filepath.Join(opts.DataDir, "dynamodb.bolt"), nil,
		func(db *bolt.DB) error { return schemaver.Ensure(db, "dynamodb", schemaver.Current) })
	if err != nil {
		return nil, err
	}
	logf := opts.Logf
...
func (s *Server) janitor() {
	...
	for {
		...
		case <-s.stop:
			return
		case <-t.C:
			bg.Tick(s.logf, "dynamodb: janitor", func() { s.store.SweepTTL() })
		}
	}
}
