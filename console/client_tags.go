...
func (b *backend) ResourceTags(ctx context.Context, svc, id string) ([]KV, error) {
	...
	case "logs":
		...
		body, e := b.json11(ctx, "Logs_20140328", "ListTagsForResource", map[string]any{"resourceArn": b.tagARN(svc, id)})
		if e == nil {
			json.Unmarshal(body, &out)
			m = out.Tags
		}
...
func (b *backend) SetResourceTag(ctx context.Context, svc, id, key, value string) error {
	...
	case "sfn":
		...
		return err
	case "logs":
		_, err := b.json11(ctx, "Logs_20140328", "TagResource", map[string]any{
