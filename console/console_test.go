...
func TestDynamoDBTableEditing(t *testing.T) {
	...
	// Counts the row CLASS rather than a literal attribute sequence: the
	// assertion is "one row", and pinning the exact attribute order made it
	// fail the moment the row gained role/tabindex for keyboard reach.
	if strings.Count(page1, `class="ddb-row"`) != 1 || !strings.Contains(page1, "Load 50 more") {
	...
	if !strings.Contains(page2, `class="ddb-row"`) {
