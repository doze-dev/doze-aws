...
func liveUnchanged(w http.ResponseWriter, r *http.Request, hash string) bool {
	...
	// when a morph swap leaves the root element's attributes untouched (morphing
	// patches children, not the root's attrs) — otherwise the poll never sees
	// its own hash advance and 200-loops forever.
