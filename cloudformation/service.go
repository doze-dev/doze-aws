...
func New(opts Options) (*Server, error) {
	...
	db, err := lazybolt.Open(filepath.Join(opts.DataDir, "cloudformation.bolt"), nil,
		func(db *bolt.DB) error { return schemaver.Ensure(db, "cloudformation", schemaver.Current) })
	if err != nil {
		return nil, err
	}
	logf := opts.Logf
