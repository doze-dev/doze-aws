...
// Lambda API takes. An s3:// reference is passed through for Lambda to fetch,
// which is what `sam deploy` and `cdk deploy` produce after packaging; a plain
// path is resolved and checked here, so a typo fails at apply rather than at
// first invoke.
func codeLocation(name, code string) (map[string]string, error) {
	if rest, ok := strings.CutPrefix(code, "s3://"); ok {
		b, k, found := strings.Cut(rest, "/")
		if !found || b == "" || k == "" {
			return nil, fmt.Errorf("function %q: %q is not a valid s3://bucket/key reference", name, code)
		}
		return map[string]string{"S3Bucket": b, "S3Key": k}, nil
	}
	abs, err := filepath.Abs(code)
	if err != nil {
		return nil, fmt.Errorf("function %q: %w", name, err)
	}
	if _, err := os.Stat(abs); err != nil {
		return nil, fmt.Errorf("function %q: code path %s does not exist", name, abs)
	}
	return map[string]string{"S3Bucket": "_local_", "S3Key": abs}, nil
}
