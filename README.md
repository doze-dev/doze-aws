...
| API Gateway | ✅ REST v1 — deployed APIs actually serve into Lambda over a real HTTP endpoint; TOKEN and REQUEST Lambda authorizers gate methods with the policy the function answers; stage access and execution logs written to CloudWatch Logs; a CDK `RestApi`'s Resource and Method tree deploys as declared | **fully audited**: 103/108 across all 36 routed operations with constrained input · 5 not expressible on the wire · no known gaps |
