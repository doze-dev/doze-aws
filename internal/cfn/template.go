// Package cfn turns CloudFormation and SAM templates into the declarative
// stack doze-aws already knows how to converge.
//
// It is internal because it is a front end, not a contract: the public
// cloudformation package is the SERVICE, and this is the transpiler it calls
// on CreateStack. cmd/doze-aws and the console call it directly for
// `doze-aws stack` and the template export.
//
...
// StackARN builds the ARN a stack reports as its StackId. It lives here
// rather than beside the store because nested.go needs it while resolving a
// child stack's references, which happens during transpilation — before any
// store exists.
