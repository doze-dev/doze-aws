...
| GetBucketPolicy / PutBucketPolicy / DeleteBucketPolicy | C | stored and returned; not evaluated as an access control. A policy that grants to everyone is refused with `AccessDenied` while the bucket's public access block has `BlockPublicPolicy` on — the one check S3 makes on the put itself |
...
policies, object ACLs and ownership controls are not consulted by S3 itself.
They round-trip so that templates and SDK code paths that set them keep
working; they do not grant or deny anything. The public access block is the
exception with a local effect: `BlockPublicPolicy` refuses a public policy at
the put, which is the mistake it exists to catch, and `GetBucketPolicyStatus`
says whether the stored policy is public.
