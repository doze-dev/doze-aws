...
| **Bucket name — formatted as an IP** | ❌ `10.0.0.1` is accepted; AWS refuses it |
| Everything else | not yet audited |

The IP-form gap is the instructive one: validation here is not missing, it
**stops partway**. That is harder to spot by reading than an absent check, and
it is why the audit works by comparing against AWS's own constraints rather than
