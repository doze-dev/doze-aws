...
**199/275 model-derived constraints enforced across all 64 routed operations**,
with 48 that cannot be put on this wire and **28 known gaps, each listed with
its cause** in `rejection_parity_test.go`. Removing the constraint table makes
144 of the 199 slip through.

This is the only service here that does not reach a clean sheet, and the gaps
are named rather than rounded away.
...
Two members are part of an operation's *identity*, not just its input:
`PUT /{Bucket}/{Key+}` is CopyObject when `x-amz-copy-source` is present and
PutObject when it is not, and `POST` on the same path is
CompleteMultipartUpload only when `?uploadId` is.

### The 48 that cannot be expressed

Emptying the last label of a URI produces a shorter path, which is a different
valid operation — `GET /bucket?tagging` is GetBucketTagging, not a
GetObjectTagging missing its key. At the service root it is stronger still: `/`
is routed by method alone, so `GET /?tagging` is ListBuckets. Both rules are
derived from the bindings rather than listed by hand.

### The 28 gaps, by cause

- **An object-only marker on a bucket path matches no route (10).**
  `GET /bucket?legal-hold` should be an error; doze-aws answers it. Nothing
  validates it because the matcher requires the object path.
- **Omitting an identity member makes it a different operation (9).**
  `PUT /b/k` without `?uploadId` really is PutObject, so UploadPart's required
  `UploadId` cannot be tested — but unlike the label cases the harness cannot
  derive that, because the distinguishing member is a header or query parameter
  rather than a path segment.
- **An emptied XML structure reads as an empty string (6).** Removing `Suffix`
  leaves `<IndexDocument/>`, which the generic parser returns as `""` rather
  than an empty structure, so the required member inside it is never looked for.
- **ListDirectoryBuckets is shadowed by ListBuckets (3).** Both are `GET /`,
  distinguished only by the advisory `x-id` the matcher ignores.
